...
type Review = {
  ...
  pro_reply: string | null; replied_at: string | null; profiles: { full_name: string | null } | null;
};
...
export default function ProReviews({ proId, proName }: { proId: string; proName: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setReviews((data ?? []).map((r: any) => ({ ...r, profiles: { full_name: names[r.author_id] ?? null } })));
          ...
          <div key={r.id} className="border-t border-neutral-100 pt-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold truncate">{r.profiles?.full_name ?? "Un habitant"}</p>
